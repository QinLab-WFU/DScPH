...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int):
        ...
        self.image_hash = LinearHash(2048,128)
...
class Text(nn.Module):
    def __init__(self, vocab_size, transformer_width, context_length, transformer_heads, transformer_layers, embed_dim):
        ...
        self.build_attention_mask = build_attention_mask(32)
    ...
    def forward(self, text):
        ...
        x = x + self.positional_embedding[:x.size(1), :].type(torch.float32)
        x = x.permute(1, 0, 2)
        x = self.transformer(x)
        x = x.permute(1, 0, 2)
