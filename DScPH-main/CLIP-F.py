...
class Basedata(Dataset):
    ...
    def __init__(self, indexs: dict, captions: dict, labels: dict):
    ...
    def get_all_label(self):
        ...
        for i, item in enumerate(self.labels):
            ...
            labels[i] = torch.from_numpy(item.astype('float32'))
        return labels
...
def load_clip(clipPath: str) -> tuple:
    try:
...
# clipPath = '/home/admin00/HYD/DHaPH/DHaPH/ViT-B-32.pt'
clipPath = '/home/abc/wulei/CPF/DCHMT-main/ViT-B-32.pt'

imageFile = '/home/abc/wulei/CPF（复件）/DCHMT-main/dataset/nuswide/index.mat'
captionFile = '/home/abc/wulei/CPF（复件）/DCHMT-main/dataset/nuswide/caption.txt'
labelFile = '/home/abc/wulei/CPF（复件）/DCHMT-main/dataset/nuswide/label.mat'

image_index = scio.loadmat(imageFile)['index']
...
data = Basedata(indexs=image_index, captions=captions, labels=labels)
data_label = data.get_all_label()
dataloder = DataLoader(dataset=data, batch_size=128, num_workers=4, pin_memory=True, shuffle=False)

img_buffer = torch.empty(195834, 512, dtype=torch.float).to(1)
text_buffer = torch.empty(195834, 512, dtype=torch.float).to(1)

embedDim, clip = load_clip(clipPath)
clip = clip.to(1)
with torch.no_grad():

    for image, text, label, index in dataloder:
        index = index.numpy()

        image = image.to(1, non_blocking=True)
        text = text.to(1, non_blocking=True)

        output_img = clip.encode_image(image)
        output_text = clip.encode_text(text)

        img_buffer[index, :] = output_img.float()
        text_buffer[index, :] = output_text.float()

result = {
    'image_f': img_buffer.cpu().detach().numpy(),
    'text_f': text_buffer.cpu().detach().numpy(),
    'label': data_label.data.numpy()
}

scio.savemat('/home/abc/wulei/CPF（复件）/DCHMT-main/dataset/nuswide/nuswide-clip.mat', result)
